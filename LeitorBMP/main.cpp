...
BFHeader bf;
//unsigned char** repBMP; //r epBMP; //Representacao binaria da imagem
// lembrando que o BMP começa de baixo para cima da esquerda para a direita
int main(int argc, char* argv[]) {

	FILE* fp;
	char arg[] = "C:\\Users\\italo\\OneDrive\\Área de Trabalho\\LeitorBmp\\italo.bmp";
	int larguratotal = 0;
	int bytesPerRow;
	int y, x;
	...
	int cont = 0;
	int quantBits = 0;
	int quantidadeBitsSeguidos = 0;
	unsigned char temp = 0;
	unsigned char bitAtual = 1; // Meu run Length Encoding ja começa considerando o bit da cor definido como branco
	...
	for (y = 0; y < bf.BIHeader.imageHeigth; y++) {
		...
		//repBMP[y] = (unsigned char*)malloc(larguratotal * sizeof(char) + 1);
		//memset(repBMP[y], '\0', (sizeof(char) * larguratotal + 1));
		quantBits = quantidadeBitsSeguidos = 0;
		for (x = 0; x < bytesPerRow; x++) {
			fread(&byteAtual, 1, 1, fp);
			for (cont = 7; cont >= 0; cont--) {
				if (quantBits < larguratotal) {
					temp = checkBit(&byteAtual, cont);
					if (temp == bitAtual) quantidadeBitsSeguidos++;
					else {
						printf("%d", quantidadeBitsSeguidos);
						quantidadeBitsSeguidos = 1;
						bitAtual = temp;
					}
					if (quantidadeBitsSeguidos >= 0xff) {
						return (0);
					}
				}
				quantBits++;
